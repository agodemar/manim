...
class AddingMoreText(Scene):
    ...
    def construct(self):
        quote = CustomTextMobject(r"Imagination is more important than knowledge\par \medskip $E=mc^2$")
        quote.set_color(RED)
        quote.to_edge(UP)
        quote2 = CustomTextMobject("A person who never made a mistake never tried anything new")
        quote2.set_color(YELLOW)
        author=CustomTextMobject(r"\textemdash Albert Einstein")
        author.scale(0.75)
...
class DotMap(Scene):
    def construct(self):
        ...
        for x in range(-7, 8):
            for y in range(-4, 5):
                annos[f"{x}{y}"] = CustomTextMobject(f"$({x}, {y})$")
...
class GuITFun(Scene):
    ...
    def construct(self):
        ...
        # GUIT_COLOR = "#004C1EFF" # unused

        see_you = CustomTextMobject(r"{\relsize{1}\firaextralight See you at}")
        see_you.to_edge(UP)
        see_you.set_color(GREEN_E)
        self.add(see_you)
        self.play(FadeIn(see_you))

        guit = CustomTextMobject(r"""
            {\relsize{2}\GuITmeeting[year=2019]}
            
            \medskip
            {\relsize{-2}\firaextralight Politecnico di Torino, 26 October 2019 --- Turin, Italy}
            """)
        # guit.set_color("#004C1EFF")
        self.add(guit)
        self.play(ApplyMethod(guit.scale,1.5))

        guit2 = CustomTextMobject(r"""
            \relsize{1}The $2^{2^{2^{\text{th}}}}$ Italian \TeX{} Users Group meeting
            
            \medskip
            {\relsize{-1}\url{guitex.org}}
            """)
        guit2.to_edge(DOWN)
